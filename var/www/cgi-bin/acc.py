...
def handleLogin():
    ...
    if username == None:
        ...
        if(session == None):
            ...
            response_cookie["SID"]["expires"] = 120
            print("Status: 301 Moved Permanently")
            print(response_cookie.output())
            print("Location: /cgi-bin/acc.py")
            print()
    else :
        if os.path.exists('user_database'):
            with open('user_database', 'rb') as f:
...
if 'HTTP_COOKIE' in os.environ:
    request_cookie = SimpleCookie()
    request_cookie.load(os.environ["HTTP_COOKIE"])

    if "SID" in request_cookie:
        print("Your Session ID is", request_cookie["SID"].value,file=sys.stderr)
        with open('sessions/session_'+ request_cookie["SID"].value, 'rb') as f:
            sess = pickle.load(f)
        printAccPage(sess)
    else:
        handleLogin()    
