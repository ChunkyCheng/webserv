...
HTTP/1.1 web server built in C++98 using Linux epoll.

## Build & Run

```bash
make
./webserv              # default: webserv.conf (port 8080)
./webserv my.conf      # custom config
```

## CGI Scripts

Scripts live in `var/www/cgi-bin/`. The `/cgi-bin` location block in `webserv.conf` handles `.py` files via `/usr/bin/python3`.
